...
# constants
dt = 1 /200   # chosen to make it run a bit faster than one day at a time (originally: 1/200) 
iterations = 100000  # number of iterations to run

# Import file with glacier data
spreadsheet = pd.read_excel('/Users/malisselummus/Downloads/Specs_03473.xlsx', sheet_name = 'Sheet1')
...
gridpoints = len(spreadsheet['Koshi_DEM'])  # Number of gridpoints we want to calculate (501 makes it about 60m) 
dx = L / gridpoints * 1000  # distance between gridpoints in meters
...
ice_elevation = spreadsheet['Koshi_DEM']
...
bed_elevation = spreadsheet['Bed_elevation']
...
def calculateFlux():
    ...
    for i in range(gridpoints - 1):
        ...
        # calculate our thickness values, taking width into consideration
...
def calculateThickness():
    for i in range(gridpoints):
        ...
        change_in_thickness = (
            (((-1 * current_upstream_flux) + current_downstream_flux) / (dx * average_width))
            + mass_balance_flux
        ) * dt
